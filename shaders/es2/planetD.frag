...
uniform highp vec3 ka;            // Reflectividad Ambiental
uniform highp vec3 ks;            // Reflectividad Especular
uniform highp float shininess;    // Factor del brillo Especular
...
{
    // Calcula el factor de la luz al fragmento
    highp vec3 s = normalize(vec3(viewMatrix * vec4(lightPosition, 1.0)) - position);

    // Calcula el vector desde el fragmento hasta la posición del ojo.
    // (origen puesto que se encuentra en el espacio "ojo" o "cámara")
    highp vec3 v = normalize(-position);

    // Refleja el haz de luz utilizando la normal en éste fragmento
    highp vec3 r = reflect(-s, normal);

    // Calcula el componente difuso
    ...
    // BUscar factores difusos y especulares
    highp vec3 diffuseColor = texture2D(diffuseTexture, flipYTexCoord).rgb;

    // Combinar las contribuciones de ambiente, difusas y especulares
